$(document).ready(function(){
	FB.init({ 
		appId:'373800562643193', cookie:true, 
	...
	(function(d, s, id) {
	  ...
	  js.src = "//connect.facebook.net/en_US/all.js#xfbml=1&appId=373800562643193";
	  fjs.parentNode.insertBefore(js, fjs);
	}(document, 'script', 'facebook-jssdk'));	
	*/
	
	//set the language override if there is one
	//$('#lang').val($('#curLang').val());
	//$('#lang').selectmenu($('#curLang').val());
...
function inviteFriends() {
	FB.ui({ method: 'apprequests', 
      message: $('#invite_your_friends').val() });
}
...
function closeInitLoader(type) {
	...
	if (bRoomInit && bChatInit && bUserInit && bLabels) {
	//	$.modal.close();

/*	
		var spinner = $("div#wait_spin");
        spinner.stop();
...
function openInitLoader(bForce) {
	if (bForce) {
		bLabels = false;
		bRoomInit = false;
		bChatInit = false; 
	...
	$.blockUI({ message: $("div#wait_spin") });
	//$("div#wait_spin").fadeIn("fast");
	
	//$("#wait_spin").modal({position: ["50%","50%"],focus:true, opacity:0, overlayCss: {backgroundColor:"#fff"}});
	
	//$("#wait_spin").modal({position: ["50%","50%"],focus:true});
}
...
$(function() {
	...
	$( "#dialog-form" ).dialog({
		...
		modal: true,
		buttons: {
			"Create a Chat Topic": function() {
				var bValid = true;
//				allFields.removeClass( "ui-state-error" );

//				bValid = bValid && checkLength( roomname, "roomname", 5, 40 );
...
//				bValid = bValid && checkRegexp( password, /^([0-9a-zA-Z])+$/, "Password field only allow : a-z 0-9" );
				...
				if ($('#roomname').val() == '' || $('#roomname').val() == null) {
					alert($('#not_entered_chat_topic').val());
					$('#roomname').focus()
					bValid = false;
				}
				else if (nCount == 0) {
					alert($('#not_selected_users').val());
					bValid = false;
				}

				if ( bValid ) {
					addRoom($('#roomname').val(), roomusers);
	...
	$('.ui-dialog-buttonpane button:contains("Create a Chat Topic")').attr("id","btn_create_chat_topic");
	$('.ui-dialog-buttonpane button:contains(" Cancel ")').attr("id","btn_create_chat_cancel");
...
function addRoom(name, users, note) {
	$.tzGET('addChatRoom',{name: name, users: users, lang: $('#lang').val(), note: note},function(r){
		working = false;
...
function reloadAndChangeRoom(r) {
	chat.getRooms();
	changeRoom(r,'');
}

function changeRoom(r,n) {
	$.tzGET('changeRoom',{room: r},function(){
		working = false;
...
var chat = {
	...
	init : function(){
		chat.login($('#name').val(),$('#email').val(),$('#country').val());
	...
	checklogged : function(name, gravatar, country) {
		chat.data.name = name;
		chat.data.gravatar = gravatar;
		chat.data.country = country;

		$.tzGET('login',{name: name, email: gravatar, country: country},function(r){
	...
	login : function(name,gravatar,country){
		openInitLoader();
...
*/
	...
	render : function(template,params){
		...
		switch(template){
			case 'loginTopBar':
				arr = [
				'<span><img src="https://graph.facebook.com/',params.gravatar,'/picture/" width="23" height="23" /> ', 
				'<span class="name">',params.name,' - <span id="roomLabel">Lobby</span></span><a href="" class="logoutButton rounded" id="logoutbutton">Logout</a></span>'];
			break; 
...
*/	
				...
				if (type == 'P') {
					...
					arr = [
					'<div class="chat-',params.id,' chat-text"><div class="main-chat-image"><img src="https://graph.facebook.com/',params.gravatar,
					'/picture/" width="26" height="26" onload="this.style.visibility=\'visible\'" /></div><div class="main-chat-message-background"><div class="main-chat-message"><span class="text',params.msgtype,'">',t,
					...
					arr = [
					'<div class="chat-',params.id,' chat-text"><div class="main-chat-image"><img src="https://graph.facebook.com/',params.gravatar,
					'/picture/" width="26" height="26" onload="this.style.visibility=\'visible\'" /></div><div class="main-chat-message-background"><div class="main-chat-message"><span class="text',params.msgtype,'">',params.trans,
			...
			case 'user':
				arr = [
					'<div class="user" title="',params.name,'"><a href="http://www.facebook.com/',params.gravatar,'" target="_blank"><img src="https://graph.facebook.com/',
					params.gravatar,'/picture/" width="30" height="30" onload="this.style.visibility=\'visible\'" border=0 /></a></div>'    
//					'<p>',params.name,'</p>'  
				];
			break;
			...
			case 'selectuser':
				arr = [
					...
					params.gravatar,'" width="30" height="30" onload="this.style.visibility=\'visible\'" onclick="javaScript:selectUser(this)"/><input type="checkbox" class="selectuser" id="',params.userID,'"></div>'  
//					'<p>',params.name,'</p>'  
				];
			break;
			...
			case 'room':
				var sRoom = params.trans.toString();
				var esc = sRoom.replace("'", "\\\'");
				
//				arr = [
//					'<div class="user" id="room',params.id,'" title="',params.trans,'" onclick="javascript:changeRoom(\'',params.id,'\', \'',esc,'\')" style="cursor:hand;cursor:pointer">',params.trans,'</div>'
//				];

				var subs = '<div><ul>';  
					
				for (var i=0; i < params.subrooms.length; i++) {
					var sub = params.subrooms[i];
					
					var sSRoom = sub.trans.toString();
					var sesc = sSRoom.replace("'", "\\\'");  
					
					subs += '<li><a href="javascript:changeRoom(\'' +  sub.id + '\',\'' + sesc + '\')">' + sub.trans + '</a></li>';
				}
				
				if (params.subrooms.length == 0) {
					subs += '<li><a href="javascript:changeRoom(\'' +  params.id + '\',\'' + esc + '\')">' + params.trans + '</a></li>';
				}	
					
				subs += '</ul></div>';

				arr = [
				'<h3><a id="room',params.id,'" title="',params.trans,'" href="javascript:changeRoom(\'',params.id,'\', \'',esc,'\')">',params.trans,'</a></h3>',subs
				];
...
//console.log('getUsers');		
	...
	getSystemTranslations : function(callback){
		$.tzGET('getSystemTranslations',{language: $('#lang').val()},function(r){
			...
			closeInitLoader('labels');
		});
	},
	
	getAllUsers : function(callback){
		$.tzGET('getAllUsers',function(r){
			var users = [];
			for(var i=0; i< r.users.length;i++){
				if(r.users[i]){
 					users.push(chat.render('selectuser',r.users[i]));
				}
			}

			$('#inviteUsers').html(users.join(''));
		});
	},
	
	// Requesting a list with all the rooms.
	
	getRooms : function(callback){
//console.log('lang=' + $('#lang').val());		
		
		$.tzGET('getRooms',{lang: $('#lang').val()},function(r){
			
			var rooms = [];
			
			//rooms.push('<div class="user" title="Lobby" onclick="javascript:changeRoom(\'0\')" style="cursor:hand;cursor:pointer">Lobby</div>');
			for(var i=0; i< r.rooms.length;i++){
				if(r.rooms[i]){
					rooms.push(chat.render('room',r.rooms[i]));
				}
			}
...
/*			
			if(r.total<1){
				room = 'No available Chat Rooms';
			}
			else {
				room = (r.total == 1 ? 'room':'rooms')+' available';
			}
*/
/*
			$.tzGET('translateText',{text: room, lang: $('#lang').val()},function(res){
				working = false;

				room = res;
				
				rooms.push('<p class="count">'+r.total+' '+room+'</p>');

				//$('#chatRooms').html(rooms.join(''));
*/
				$('#accordion').html(rooms.join(''));
				
				$('#room' + $('#chatRoom').val()).click();	
